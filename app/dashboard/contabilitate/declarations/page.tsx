  ...
  return <div>
    ...
    {report&&<>
      ...
      <div className="declaration-grid">
        <DeclarationCard code="D300" title="Decont TVA" due={report.d300.dueDate} ready={report.d300.ready} href={exportUrl("D300")}><div className="declaration-values"><span>TVA colectată<strong>{money(report.d300.outputVat)} RON</strong></span><span>TVA deductibilă<strong>{money(report.d300.inputVat)} RON</strong></span><span>{report.d300.vatPayable>0?"TVA de plată":"TVA de recuperat"}<strong>{money(report.d300.vatPayable||report.d300.vatRefundable)} RON</strong></span></div></DeclarationCard>
        <DeclarationCard code="D394" title="Operațiuni interne" due={report.d394.dueDate} ready={report.d394.ready} href={exportUrl("D394")}><div className="declaration-values"><span>Clienți raportați<strong>{report.d394.sales.length}</strong></span><span>Achiziții înregistrate<strong>{report.d394.purchases.length}</strong></span></div></DeclarationCard>
        <DeclarationCard code="D390" title="Operațiuni UE" due={report.d390.dueDate} ready={report.d390.ready} href={exportUrl("D390")}><div className="declaration-values"><span>Parteneri UE<strong>{report.d390.sales.length}</strong></span><span>Bază raportată<strong>{money(report.d390.sales.reduce((s,r)=>s+r.taxableBase,0))} RON</strong></span></div></DeclarationCard>
      </div>
...
function DeclarationCard({code,title,due,ready,href,children}:{code:string;title:string;due:string;ready:boolean;href:string;children:React.ReactNode}){
  return <div className="card declaration-card"><div className="declaration-card-head"><div><span>{code}</span><h3>{title}</h3></div>{ready?<span className="declaration-ready"><CheckCircle2 size={14}/>Date complete</span>:<span className="declaration-review"><AlertTriangle size={14}/>Necesită verificare</span>}</div>{children}<div className="declaration-card-foot"><span>Termen orientativ: <strong>{due}</strong></span><a className="btn-secondary" href={href}><Download size={14}/>Fișă CSV</a></div></div>
}
