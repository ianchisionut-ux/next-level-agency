...
export default function DeclarationsPage() {
  ...
  return <div>
    ...
    <div className="ref-notice"><strong>Flux oficial:</strong> butonul „Formular ANAF” descarcă PDF-ul inteligent publicat de ANAF și valabil pentru perioada selectată. Fișa de control și CSV-ul conțin valorile calculate în Facturare. Formularul completat trebuie validat, semnat digital și transmis, iar recipisa trebuie înregistrată în dosarul contabil.</div>
    {report&&<>
      ...
      <div className="declaration-grid">
        <DeclarationCard code="D300" title="Decont TVA" due={report.d300.dueDate} ready={report.d300.ready} csvHref={exportUrl("D300")} pdfHref={pdfUrl("D300")} officialHref={officialUrl("D300")} official={report.officialForms.D300}><div className="declaration-values"><span>TVA colectată<strong>{money(report.d300.outputVat)} RON</strong></span><span>TVA deductibilă<strong>{money(report.d300.inputVat)} RON</strong></span><span>{report.d300.vatPayable>0?"TVA de plată":"TVA de recuperat"}<strong>{money(report.d300.vatPayable||report.d300.vatRefundable)} RON</strong></span></div></DeclarationCard>
        ...
        <DeclarationCard code="D390" title="Operațiuni UE" due={report.d390.dueDate} ready={report.d390.ready} csvHref={exportUrl("D390")} pdfHref={pdfUrl("D390")} officialHref={officialUrl("D390")} official={report.officialForms.D390}><div className="declaration-values"><span>Parteneri UE<strong>{report.d390.sales.length}</strong></span><span>Bază raportată<strong>{money(report.d390.sales.reduce((s,r)=>s+r.taxableBase,0))} RON</strong></span></div></DeclarationCard>
      </div>
...
function DeclarationCard({code,title,due,ready,csvHref,pdfHref,officialHref,official,children}:{code:string;title:string;due:string;ready:boolean;csvHref:string;pdfHref:string;officialHref:string;official:{version:string;legalBasis:string;sourcePageUrl:string};children:React.ReactNode}){
  return <div className="card declaration-card"><div className="declaration-card-head"><div><span>{code}</span><h3>{title}</h3></div>{ready?<span className="declaration-ready"><CheckCircle2 size={14}/>Date complete</span>:<span className="declaration-review"><AlertTriangle size={14}/>Necesită verificare</span>}</div><div className="declaration-official-version"><strong>Formular oficial v{official.version}</strong><span>{official.legalBasis}</span><a href={official.sourcePageUrl} target="_blank" rel="noreferrer">Sursa ANAF <ExternalLink size={11}/></a></div>{children}<div className="declaration-card-foot"><span>Termen orientativ: <strong>{due}</strong></span><div className="declaration-downloads"><a className="btn-primary" href={officialHref}><Download size={14}/>Formular ANAF</a><a className="btn-secondary" href={pdfHref}><Download size={14}/>Fișă control</a><a className="btn-secondary" href={csvHref}><Download size={14}/>CSV</a></div></div></div>
}
