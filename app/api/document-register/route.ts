...
export async function GET() {
  ...
  return NextResponse.json({ documents: documents.map(serialize) });
}
  ...
  try {
    ...
    const document = await prisma.companyDocument.create({
      data: {
        ...
        createdByName: check.user.name,
      },
    });

    return NextResponse.json({ document: serialize(document) }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Nu am putut înregistra actul.";
