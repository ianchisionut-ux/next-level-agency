...
function config() {
  ...
  return { clientId: process.env.ANAF_CLIENT_ID || "", clientSecret: process.env.ANAF_CLIENT_SECRET || "", redirectUri: process.env.ANAF_REDIRECT_URI || "https://www.nextlevel-agency.ro/api/accounting/efactura/callback", environment, apiBase: API_BASES[environment] };
}
...
async function recordPermanentInvoiceError(invoiceId: number, message: string) {
  ...
  try {
    ...
    const latest = (await connection.query(
      `SELECT id,status,message FROM efactura_submissions WHERE "invoiceId"=$1 ORDER BY id DESC LIMIT 1`,
      [invoiceId]
    )).rows[0];
    if (!latest || latest.status !== "ERROR" || latest.message !== message) {
      const attempt = Number((await connection.query(
        `SELECT COALESCE(MAX("attemptNumber"),0)+1 AS attempt FROM efactura_submissions WHERE "invoiceId"=$1`,
        [invoiceId]
      )).rows[0].attempt);
      await connection.query(
        `INSERT INTO efactura_submissions ("invoiceId",status,message,"xmlSnapshot",retryable,"attemptNumber","checkedAt")
         VALUES ($1,'ERROR',$2,'',0,$3,now())`,
        [invoiceId, message, attempt]
      );
    }
...
export async function submitEFactura(invoiceId: number, xml: string, cif: string) {
  ...
  try {
    ...
    const { rows } = await connection.query(
      `INSERT INTO efactura_submissions ("invoiceId",status,"xmlSnapshot",retryable,"attemptNumber")
       VALUES ($1,'UPLOADING',$2,0,$3) RETURNING id`,
      [invoiceId, xml, attempt]
    );
  ...
  try {
    ...
    if (!response.ok) {
      throw new AnafSubmissionError(responseText.slice(0, 500) || `ANAF a răspuns cu HTTP ${response.status}.`, response.status === 429 || response.status >= 500);
    }
    if (!uploadId) throw new AnafSubmissionError(responseText.slice(0, 500) || "ANAF nu a returnat identificatorul încărcării.", true);
    await pool.query(
    ...
    await pool.query(
      ...
      [error instanceof Error ? error.message : String(error), isRetryableError(error) ? 1 : 0, submissionId]
    );
    throw error;
  }
}
...
export async function sendInvoiceToAnaf(invoiceId: number) {
  ...
  try {
    ...
    await recordPermanentInvoiceError(invoiceId, error instanceof Error ? error.message : String(error));
    throw error;
  }
...
export async function checkSubmission(id: number) {
  const pool = await ready();
  const { rows } = await pool.query(`SELECT * FROM efactura_submissions WHERE id=$1`, [id]);
  const item = rows[0];
  if (!item?.uploadId) throw new Error("Transmiterea nu are ID ANAF.");
  const response = await anafFetch(`/stareMesaj?id_incarcare=${encodeURIComponent(item.uploadId)}`);
  const responseText = await response.text();
  if (!response.ok) throw new AnafSubmissionError(responseText.slice(0, 500) || `ANAF a răspuns cu HTTP ${response.status}.`, response.status === 429 || response.status >= 500);
  const lower = responseText.toLowerCase();
  const status: EFacturaStatus = lower.includes("nok") || lower.includes("eroare") ? "REJECTED" : lower.includes("ok") ? "VALIDATED" : "PROCESSING";
...
export async function latestSubmission(invoiceId: number) {
  const { rows } = await (await ready()).query(
    `SELECT id,"uploadId",status,message,"downloadId","submittedAt","checkedAt",retryable,"attemptNumber"
       FROM efactura_submissions WHERE "invoiceId"=$1 ORDER BY id DESC LIMIT 1`,
    [invoiceId]
  );
  return rows[0] || null;
}
...
export async function processAutomaticEFactura(limit = 20) {
  ...
  await pool.query(
    `UPDATE efactura_submissions
      ...
      WHERE status='UPLOADING' AND "createdAt" < now() - interval '1 hour'`
  );

  const processing = await pool.query(
    `SELECT id FROM efactura_submissions WHERE status='PROCESSING'
