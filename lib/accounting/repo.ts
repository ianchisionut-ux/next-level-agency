  ...
  try {
    ...
    await connection.query(
      ...
      [input.invoiceType ?? "STANDARD", input.originalInvoiceId ?? null, input.stornoReason ?? "", input.initialStatus ?? "issued",
        ...
        JSON.stringify(input.sellerSnapshot || companyResult.rows[0] || {}), JSON.stringify(input.clientSnapshot || clientResult.rows[0]), 1, invoiceId]
    );
    ...
    for (const item of computed) {
      await connection.query(
        ...
        [invoiceId, item.productId ?? null, item.description, item.um, item.qty, item.unitPrice, item.vatRate, item.valoare, item.vatValue,
          item.unitCode || "H87", item.vatCategoryCode || (item.vatRate === 0 ? "Z" : "S"), item.taxExemptionReasonCode || "", item.taxExemptionReason || ""]
      );
    }
